...
`${WALLET_MNEMONIC}` makes that variable required only in configurations that reference it. `passphrase` defaults to an empty string. The shipped `config/store.toml` configures wallets and therefore requires `WALLET_MNEMONIC`; registration derives and persists one Base and one Solana address for each user.
:::
