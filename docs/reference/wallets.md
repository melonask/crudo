...
`${WALLET_MNEMONIC}` makes that variable required only in configurations that reference it. `passphrase` defaults to an empty string. The shipped configurations omit `WALLET_PASSPHRASE`.
:::
