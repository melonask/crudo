...
`--config` selects a local path or HTTPS URL explicitly. Without it, crudo reads `./Crudo.toml`. If neither is available, startup fails with guidance; unreadable or invalid selected configuration also fails startup. Installed binaries do not depend on the source-tree `config/store.toml`.
...
- The shipped store configuration configures wallets and ALTCHA.

::: info Conditional feature secrets
A configuration with wallet stages may require `${WALLET_MNEMONIC}`; one with `[altcha]` requires its configured secrets. Neither is globally required by crudo. The shipped `config/store.toml` requires `${DATABASE_URL}`, `${WALLET_MNEMONIC}`, `${ALTCHA_SECRET}`, and `${ALTCHA_KEY_SECRET}`.
:::
